  ...
  try {
    ...
    if (!valid.success) {
      return NextResponse.json(
        { success: false, message: valid.error },
        { status: 400 }
      );
    }
    ...
    if (!team) {
      return NextResponse.json(
        {
          success: false,
          mmessage: 'No such team is found with the given teamId',
        },
        { status: 501 }
      );
    }
    ...
    if (team.members.length === 4) {
      const hasFemaleMember = team.members.some(
      ...
      if (!hasFemaleMember && memberdetails.gender !== 'female') {
        return NextResponse.json(
          {
            success: false,
            message: 'At least one female student should be in a team',
          },
          { status: 501 }
        );
      }
